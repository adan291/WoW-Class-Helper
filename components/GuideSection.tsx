...
const GuideSection = ({ title, icon, isLoading, content, error }: GuideSectionProps) => {
  ...
  const processInlineMarkdown = (text: string) => {
    ...
    processed = processed.replace(tooltipRegex, (match, ability, content) => {
        // Extract Cooldown, ID, and Description using regex
        // Format expected: Cooldown: X sec. ID: 123. Description: text
        ...
        // If description isn't explicitly found, fallback to the whole content if it doesn't look like metadata
        const description = descMatch ? descMatch[1].trim() : (content.includes('Cooldown') ? '' : content);

        // Generate a "Fake Icon" based on the first letter of the ability
