...
const ApplicantModal: React.FC<{
  ...
  return (
    ...
    >
      <div
        className="wow-panel w-full max-w-2xl max-h-[90vh] overflow-y-auto font-cinzel"
      ...
      >
        <div className="p-6 border-b border-wow-brown-dark/20 dark:border-wow-gray-700 flex justify-between items-center">
          <div className="flex items-center gap-4">
            <img
              src={classData?.icon}
              alt={applicant.wowClass}
              className="w-12 h-12 rounded-full border-2"
              style={{ borderColor: classData?.color }}
            />
            <div>
              <h2 className="text-2xl font-bold" style={{ color: classData?.color }}>
                {applicant.playerName}
              </h2>
              <p className="text-wow-brown dark:text-gray-400">
                {applicant.wowSpec} {applicant.wowClass} - {applicant.itemLevel} ilvl
              </p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-4 text-wow-brown-dark dark:text-gray-300">
          <div>
            <strong className="text-wow-brown-light dark:text-wow-gold">{t('modal_role')}:</strong>{' '}
            {applicant.role}
          </div>
          <div>
            <strong className="text-wow-brown-light dark:text-wow-gold">{t('modal_logs')}:</strong>{' '}
            <a
              href={applicant.logsUrl}
              target="_blank"
          ...
          <div>
            <strong className="text-wow-brown-light dark:text-wow-gold">
              {t('modal_raidExp')}:
            </strong>{' '}
            <ul className="list-disc list-inside ml-4">
              {applicant.raidExperience.map((e) => (
                <li key={e}>{t(e as keyof typeof t)}</li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-1">
              {applicant.availability.map((d) => (
                <span
                  key={d}
                  className="bg-wow-parchment-dark dark:bg-wow-gray-700 px-2 py-1 text-xs rounded"
                >
          ...
          <div>
            ...
            <p className="bg-wow-parchment dark:bg-wow-gray-900 p-3 rounded-md mt-1 italic">
              "{applicant.reason}"
            </p>
          </div>
          {guild?.customQuestions && guild.customQuestions.length > 0 && (
            <div className="border-t border-wow-brown-dark/20 dark:border-wow-gray-700 pt-4">
              <strong className="text-wow-brown-light dark:text-wow-gold mt-4 block">
                {t('admin_custom_questions_header')}
              </strong>
              {guild.customQuestions.map((q) => (
                <div key={q.id} className="mt-2">
                  <p className="font-semibold text-wow-brown dark:text-gray-400">{q.label}</p>
                  <p className="bg-wow-parchment dark:bg-wow-gray-900 p-3 rounded-md mt-1 italic">
                    "{applicant.customAnswers?.[q.id] || t('admin_custom_questions_no_answer')}"
                  </p>
                </div>
              ))}
            </div>
        ...
        <div className="p-6 bg-wow-parchment-dark/50 dark:bg-wow-gray-900/50 flex justify-between items-center gap-3">
          <button onClick={handleEdit} className="wow-button !text-sm !py-2 !px-4">
            {t('modal_edit_button')}
          </button>
  ...
  const getStatusIndicatorClass = (status: ApplicationStatus) => {
    switch (status) {
      ...
      case ApplicationStatus.REJECTED:
        return 'bg-red-500';
      default:
        return 'bg-yellow-500 animate-pulse-slow';
    }
  };
    ...
    <div className="font-cinzel">
      {selectedApplicant && (
        <ApplicantModal
          ...
          onStatusChange={context.updateApplicationStatus}
        />
      )}
      <div className="wow-panel">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-wow-brown-dark dark:text-wow-gold tracking-wider">
            {context.authenticatedUser.guildName} {t('admin_dashboard_header')}
          </h1>
        </div>
        <div className="my-6 p-4 bg-wow-parchment-dark/50 dark:bg-wow-gray-900/50 rounded-md border border-wow-brown-light/50 dark:border-wow-gray-700 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <select
              ...
              defaultValue="submissionDate_desc"
              className="wow-input w-full"
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <select
              onChange={(e) => setFilters({ ...filters, raidExperience: e.target.value })}
              className="wow-input w-full"
            >
              <option value="">All Raid Experience</option>
              {RAID_EXPERIENCES.map((exp) => (
                <option key={exp} value={exp}>
                  {t(exp as keyof typeof t)}
                </option>
              ))}
            </select>
            <select
        ...
        <div className="overflow-x-auto">
            ...
            <tbody>
              {sortedAndFilteredApps.map((app) => {
                ...
                return (
                  <tr
                    key={app.id}
                    className={`border-b border-wow-brown-dark/20 dark:border-wow-gray-700 hover:bg-wow-parchment-dark dark:hover:bg-wow-gray-700/50 transition-all duration-200 hover:scale-[1.01] ${app.isNew ? 'bg-wow-blue/10' : ''}`}
                  >
                    <td className="p-3 font-bold">
                      <div className="flex items-center gap-3">
                        <span
                          className={`h-2.5 w-2.5 rounded-full flex-shrink-0 ${getStatusIndicatorClass(app.status)}`}
                        ></span>
                        <span style={{ color: classData?.color }}>{app.playerName}</span>
                      </div>
                    </td>
                    <td className="p-3 hidden md:table-cell" style={{ color: classData?.color }}>
                      {app.wowSpec} {app.wowClass}
                    </td>
                    <td className="p-3">{app.role}</td>
                    <td className="p-3 hidden md:table-cell">{app.itemLevel}</td>
                    <td className="p-3">
                      <span
                        className={`px-2 py-1 text-xs font-bold rounded-full ${app.status === ApplicationStatus.ACCEPTED ? 'bg-green-500/20 text-green-300' : app.status === ApplicationStatus.REJECTED ? 'bg-red-500/20 text-red-300' : 'bg-yellow-500/20 text-yellow-300'}`}
                      >
          ...
          {sortedAndFilteredApps.length === 0 && (
            <p className="text-center text-wow-brown-dark dark:text-gray-400 py-8">
              {t('admin_no_results')}
            </p>
          )}
        </div>
